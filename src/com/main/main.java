...
import com.models.gameEnemy;
import com.models.gamePlayer;
import com.models.gameShoot;
...
public class main extends BaseGameActivity implements IScrollDetectorListener, IOnSceneTouchListener  {
	...
	private gameEnemy  sampleEnemy;
	private gameEnemy  SinglePlayer2;
	private Scene tscene;  
	private BaseGameActivity tactivity;
	...
	public void onLoadResources() {
		...
		// Diem que es el target player
		gameCommons.targetPlayer=singlePlayer;
		
		// Crear enemigo
		sampleEnemy = new gameEnemy(0,100,100,"warrior",gameCommons.loadToSpriteMap(gameCommons.WarriorAtlas,"images/warrior_body.png", 0, 0, 7, 1, this)); //448x64
		SinglePlayer2=new gameEnemy(0,100,100,"warrior",gameCommons.loadToSpriteMap(gameCommons.WarriorAtlas,"images/DBz.png", 0, 449, 5, 1, this));	
		// Shared IMAGES
		gameCommons.shootTexture=gameCommons.loadToSpriteMap(gameCommons.WarriorAtlas,"images/arrow.png", 0, 193, 2, 1,tactivity);
		gameCommons.shootTexture2=gameCommons.loadToSpriteMap(gameCommons.WarriorAtlas,"images/bolas.png", 0, 193, 2, 1,tactivity);
	...
	public Scene onLoadScene() {
		...
		// Por todas las partes del Single player ....
		for (gameAnimatedPart curr_part : sampleEnemy.parts)
    	{
			scene.getLastChild().attachChild(curr_part);
    	}
		for (gameAnimatedPart curr_part : SinglePlayer2.parts)
    	{
			scene.getLastChild().attachChild(curr_part);
    	}
	...
	public void onLoadComplete() 
	{
		...
		tscene.registerUpdateHandler(new IUpdateHandler() 
        {
			// RECURSIVO
			public void onUpdate(float pSecondsElapsed) {
				 ...
				 sampleEnemy.move(10);
				 SinglePlayer2.move(20);
				 // TENGO QUE DISPARAR????
				 if (System.currentTimeMillis() - startTime >= 1000 ) 
		    	 {
					    
						singlePlayer.rotate(20);
						gameShoot tshoot=new gameShoot(1,singlePlayer.body.getX(),singlePlayer.body.getY(),gameCommons.shootTexture,singlePlayer.body.getRotation(),80);
						gameShoot tshoot2=new gameShoot(2,SinglePlayer2.body.getX(),SinglePlayer2.body.getY(),gameCommons.shootTexture2,SinglePlayer2.body.getRotation(),80);
				        tscene.getLastChild().attachChild(tshoot.shootBody);
				        tscene.getLastChild().attachChild(tshoot2.shootBody);
						tscene.getLastChild().sortChildren();	
